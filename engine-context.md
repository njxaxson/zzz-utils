...
| **Remielle** | Titled T0 | Anomaly (Lumen) + `pseudoRole: ["dps", conditional "support"]` | `conditionalBuffs.atk: [0,0,2,4]` by anomaly count; `damage["ultimate:double"]: 3, "luminize": 3, "aftershock": 2` (aftershock tentative); `scaling: { ap: 3, atk: 3 }`; `onfield: "shared"` | Lumen anomaly void hunter. Faction: Covenant of Dayat. Her ATK buff (+1600 ATK at 4000 ATK, game's highest — 40% of her personal ATK stat) and support identity both scale with team composition. `support` pseudo-role activates only on triple-anomaly teams; `dps` always activates. Scales heavily on AP (anomaly rebound multiplier) and ATK (buff percentage + personal damage). Signature weapon gives 743 base ATK + 105 AP, enabling 4000+ ATK / 600+ AP — far beyond typical anomaly agents. **Luminize / Anomaly Rebound**: tracks last 3 attribute mutations, combines their Refringe-boosted damage, multiplies by AP, deals as one colossal luminize hit. **Refringe cascade**: multi-element partners preferred — Alice/Vivian/Rem (disorders cascade) > Alice/Jane/Rem (same-element, no cascade). **Shared field time** (`mechanics.onfield: "shared"`): cycles on/off-field; counted as 0.5 for field time economy. **Defensive assist** intermittently unavailable when forced off-field — counts toward boss assist requirements only when `boss.assists === team.length`. Best teams: Velina/Remielle/Promeia, Alice/Vivian/Remielle (multi-element for cascade). |
...
* `onfield` — Explicit on-field demand override. Accepts `true`, `false`, or `"shared"`. Defaults: attack/anomaly/rupture/stun = `true`; support/defense = `false`. When a unit's pseudoRole activates as a different role, on-field status is derived from the activated role unless explicitly overridden. `"shared"` marks a unit with a forced on/off-field cycle (e.g., Remielle) — they count as 0.5 for field time economy rather than 1.
...
The latest mechanics updates are for Patch 3.1 (Remielle / Lumen element release):
...
- Shared field time (`onfield: "shared"`) counts as 0.5 for field time economy. Defensive assist from shared-field units counts toward boss requirements only in the "zero evasive assists" case.
