        ...
        for (const unitB of units) {
            ...
            if (ab) {
                ...
                if (ba) {
                    ...
                    permutations[unitA.id + unitB.id] = [unitA, unitB];
                }
                ...
                for (const unitC of units) {
                    ...
                    if (a && b && c) {
                        permutations[unitA.id + unitB.id + unitC.id] = [unitA, unitB, unitC];
                    }
                }
            }
        }
 ...
 */
const ROLE_ORDER = ["stun", "anomaly", "attack", "rupture", "defense", "support"];
...
/**
 * Checks if two teams share any units (based on id assigned by getTeams)
 */
function teamsOverlap(team1, team2) {
 ...
 */
function extendTeamsWithUniversalUnits(twoCharTeams, threeCharTeams, universalUnits) {
    let extendedCount = 0;
 ...
 */
function findExclusiveCombinations(viableTeamsByBoss, bossNames) {
    const combinations = [];
